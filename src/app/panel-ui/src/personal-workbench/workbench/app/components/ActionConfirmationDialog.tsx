...
export function ActionConfirmationDialog(props: {
  ...
  return createPortal(
    <div className="ui-action-confirmation-layer">
      <button
        type="button"
        className="ui-action-confirmation-backdrop"
        aria-label="取消确认"
      ...
      <div
        ref={dialogRef}
        className="ui-action-confirmation-dialog"
        role="alertdialog"
        aria-modal="true"
        ...
        data-tone={tone}
      >
        <header className="ui-action-confirmation-header">
          <span className="ui-action-confirmation-icon" aria-hidden="true">
            <AlertTriangle size={18} strokeWidth={1.8} />
          </span>
          <div className="ui-action-confirmation-eyebrow">
            <span>{props.request.eyebrow}</span>
            <small>需要确认</small>
          </div>
          <button
            type="button"
            className="ui-action-confirmation-close"
            aria-label="取消确认"
            onClick={props.onCancel}
            disabled={busy}
          >
        ...
        <div className="ui-action-confirmation-copy">
        ...
        {props.request.consequence !== undefined && (
          <div className="ui-action-confirmation-consequence">
            <AlertTriangle size={14} aria-hidden="true" />
            <span>{props.request.consequence}</span>
          </div>
        )}

        <footer className="ui-action-confirmation-actions">
          <button
            type="button"
            className="ui-action-confirmation-cancel"
            data-confirm-cancel
            onClick={props.onCancel}
            disabled={busy}
          >
          ...
          <button
            type="button"
            className="ui-action-confirmation-confirm"
            data-confirm-submit
            data-tone={tone}
            onClick={confirm}
            disabled={busy}
