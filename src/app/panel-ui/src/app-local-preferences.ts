import { PRODUCT_DATA_FORMAT_ID } from "../../../platform/product-identity";
...
function readDesktopLocalPreference(key: string): string | undefined {
  ...
  try {
    return window.synechHost?.getLocalPreference(key);
  } catch {
    return undefined;
  }
}
...
function writeDesktopLocalPreference(key: string, value: string): boolean {
  ...
  try {
    return window.synechHost?.setLocalPreference(key, value) === true;
  } catch {
    return false;
  }
}
