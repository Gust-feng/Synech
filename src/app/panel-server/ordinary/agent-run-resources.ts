...
>(
  ...
  const [aiEnvironment, webSearch] = await Promise.all([
    runtime.configCenter.createModelRuntimeEnvironment({
      modelProvider: input.capabilitySnapshot.activeModel,
      informationAccess: input.informationAccess,
    }),
    runtime.configCenter.resolveWebSearchRuntimeConfig(input.informationAccess.web),
  ]);
  return prepareAgentHostRunResourcesWithEnvironment(runtime, input, aiEnvironment, webSearch);
}
...
>(
  ...
  webSearch: WebSearchRuntimeConfig | undefined,
): Promise<AgentHostRunResources<TCapabilitySnapshot>> {
