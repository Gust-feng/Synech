...
import { DataMaintenanceError } from "./data-maintenance.js";

export type DataRouteDependencies = {
  readonly dataMaintenance: {
...
export async function handlePanelDataRoute(
  dependencies: DataRouteDependencies,
  request: IncomingMessage,
  response: ServerResponse,
  url: URL,
): Promise<boolean> {
  if (url.pathname === "/api/data/health" && request.method === "GET") {
    const health = dependencies.dataMaintenance.health();
    writeJson(response, health.ok ? 200 : 503, { ok: health.ok, health });
    return true;
  }
  if (url.pathname === "/api/data/backups" && request.method === "POST") {
    writeJson(response, 201, { ok: true, backup: await dependencies.dataMaintenance.createBackup() });
    return true;
  }
  if (url.pathname === "/api/data/restore/select" && request.method === "POST") {
    writeJson(response, 200, { ok: true, result: await dependencies.dataMaintenance.selectAndStageRestore() });
    return true;
  }
  return false;
}

export function dataMaintenanceHttpError(error: DataMaintenanceError): PanelHttpError {
  switch (error.code) {
