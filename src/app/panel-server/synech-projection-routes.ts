...
import type { SynechProjectionChange } from "../panel-api-contracts.js";
...
export type SynechProjectionRouteDependencies = {
  readonly synechProjectionChanges: {
    subscribe(listener: (change: SynechProjectionChange) => void): () => void;
    replay(cursor?: number): { readonly changes: readonly SynechProjectionChange[]; readonly cursor: number };
  };
};

export async function handleSynechProjectionRoute(
  dependencies: SynechProjectionRouteDependencies,
  ...
  const buffered: SynechProjectionChange[] = [];
  const writer = new SseResponseWriter(response, { onFailure: () => cleanup() });
  const unsubscribe = dependencies.synechProjectionChanges.subscribe((change) => {
  ...
  try {
    const replay = dependencies.synechProjectionChanges.replay(requestCursor(request, url));
    ...
    if (!(await writer.write(": synech projection changes\n\n"))) {
      cleanup();
      return true;
    }
    ...
    heartbeat = setInterval(() => {
      if (!writer.enqueue("event: synech.projection.heartbeat\ndata: {}\n\n")) cleanup();
...
function frame(change: SynechProjectionChange): string {
  return `id: ${change.revision}\nevent: synech.projection.changed\ndata: ${JSON.stringify(change)}\n\n`;
}
